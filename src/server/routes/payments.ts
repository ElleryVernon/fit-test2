...
/**
 * Studio Payments API 라우트
 * 토스페이먼츠 결제 처리 및 구독 정보 저장
 */
export const paymentsRoutes = new Elysia({ prefix: "/studio/payments" })
  // POST /api/studio/payments/confirm - 결제 확인 및 처리
  .post(
    "/confirm",
    async ({ body, set }) => {
      try {
        ...
        if (!tossResponse.ok) {
          ...
          set.status = tossResponse.status;
          return {
            message: "결제 확인 실패",
    ...
    },
    {
      body: t.Object({
        ...
        serviceType: t.Literal("studio_program"),
      }),
    }
  );
